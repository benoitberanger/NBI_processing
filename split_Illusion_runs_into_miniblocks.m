...
raw_json_ref = get_subdir_regex_files(Illusion_dirs_ref,'^dic.*.json'); % char(raw_images)
...
%     do_delete(mb_dir{mb},0);
    ...
    mb_dir_ref{mb} = r_mkdir( subject_dirs, sprintf('miniblock_%.2d_ref',mb)); %char(mb_dir{mb})
...
for subj = 1 : length(subject_dirs)
    ...
    for run = 1 : size(raw_images{subj},1)
        ...
        r_movefile(...
            ...
            {sprintf('%sf_miniblock_%.2d_ref.nii',mb_dir_ref{mb}{subj},mb)},...
            'copy');
        ...
        % copy json ref
        r_movefile(...
            {raw_json_ref{subj}(run,:)},...
            {sprintf('%s',mb_dir_ref{mb}{subj})},...
            'copy');
